...
Прочитайте [Аудиомаршруты](audio-routes_RU.md), чтобы понять, почему WAV является reference transport и что требуется до статуса supported для live route. Формат объекта описан в [спецификации ADLP v1](../../spec/protocol-v1_RU.md), а experimental carrier compatibility — в [Acoustic-1 RFC](../../spec/acoustic-1_RU.md).
